<template>
    <div :class="`${$options.name} w-full flex text-white font-tahoma`">
        <div :class="`${$options.name}__start`">
            <img
            src="@/assets/start.png"
            alt="Start">
        </div>
        <div class="flex overflow-hidden px-2 py-2 text-xs">Active Windows</div>
        <div :class="`${$options.name}__tray flex ml-auto max-w-xs overflow-hidden text-xs py-2 justify-between`">
            <div class="mx-2">Tray Items</div>
...
export default {
    ...
    mounted() {
        setInterval(() => { this.time = this.getTime() }, 5000)
    },
    methods: {
        getTime() {
            const date = new Date()
